...
void AnimaGeometryCreator::MakePlane(AnimaGeometry* geometry, AnimaAllocator* allocator)
{
	geometry->ClearAll();
	...
	geometry->SetVertices(vertici, numeroVertici);
	geometry->SetNormals(normali, numeroVertici);
	geometry->SetTextureCoords(textCoords, numeroVertici);
	geometry->SetFaces(facce, numeroFacce);
...
void AnimaGeometryCreator::MakeCube(AnimaGeometry* geometry, AnimaAllocator* allocator)
{
	geometry->ClearAll();
	...
	geometry->SetVertices(vertici);
	geometry->SetNormals(normali);
	geometry->SetTextureCoords(textCoords);
	geometry->SetFaces(facce);
}

void AnimaGeometryCreator::MakeCylinder(AnimaGeometry* geometry, AFloat topRadius, AFloat bottomRadius, AFloat height, AUint radialSegments, AUint heightSegments, bool openEnded, AnimaAllocator* allocator)
{
	geometry->ClearAll();
...
//	AFloat tanTheta = ((AFloat)bottomRadius - (AFloat)topRadius) / (AFloat)height;
	...
	AnimaVertex3f* geometryVertici = AnimaAllocatorNamespace::AllocateArray<AnimaVertex3f>(*allocator, vertici.size());
	AnimaFace* geometryFacce = AnimaAllocatorNamespace::AllocateArray<AnimaFace>(*allocator, facce.size());

	AInt offset = 0;
	for (auto vertice : vertici)
		geometryVertici[offset++] = vertice;

	offset = 0;
	for (auto faccia : facce)
	{
		geometryFacce[offset++].SetIndexes(faccia->GetIndexes());
		AnimaAllocatorNamespace::DeallocateObject(*allocator, faccia);
		faccia = nullptr;
	}

	geometry->SetVertices(geometryVertici, (AInt)vertici.size());
	geometry->SetFaces(geometryFacce, (AInt)facce.size());

	vertici.clear();
	facce.clear();
	AnimaAllocatorNamespace::DeallocateArray(*allocator, geometryVertici);
	AnimaAllocatorNamespace::DeallocateArray(*allocator, geometryFacce);
}

void AnimaGeometryCreator::MakeIcosahedralSphere(AnimaGeometry* geometry, AUint recursionLevel, AnimaAllocator* allocator)
{
	geometry->ClearAll();
...
AInt AnimaGeometryCreator::GetMiddlePoint(AInt p1, AInt p2, std::vector<AnimaVertex3f>& vertices, std::map<__int64, int>& cache, AInt& index)
{
...
AInt AnimaGeometryCreator::AddVertex(AnimaVertex3f vertex, std::vector<AnimaVertex3f>& vertices, AInt& index)
{
	vertices.push_back(vertex.Normalized());
	return index++;
}

void AnimaGeometryCreator::AddFace(std::vector<AnimaFace*>& facce, AInt v1, AInt v2, AInt v3, AnimaAllocator* allocator)
{
	AnimaFace* face = AnimaAllocatorNamespace::AllocateNew<AnimaFace>(*allocator);
	face->SetIndexes(v1, v2, v3);
