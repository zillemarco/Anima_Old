...
static bool parseVersionString(int* api, int* major, int* minor, int* rev)
{
	...
	*api = ANIMA_ENGINE_OPENGL_API;
	...
	for (int i = 0; prefixes[i]; i++)
	{
		...
		{
			version += length;
			*api = ANIMA_ENGINE_OPENGL_ES_API;
			break;
		}
	}
...
void _AnimaEngineWindowInputMonitorChange(AnimaEngine* engine)
{
	...
	for (i = 0; i < monitorCount; i++)
	{
		...
		if (AnimaEngine::_callbacks._monitor)
			AnimaEngine::_callbacks._monitor((AnimaEngineWindowmonitor*)monitors[i], ANIMA_ENGINE_DISCONNECTED);
	}
	...
	for (i = 0; i < AnimaEngine::_monitorCount; i++)
	{
		...
		if (AnimaEngine::_callbacks._monitor)
			AnimaEngine::_callbacks._monitor((AnimaEngineWindowmonitor*)AnimaEngine::_monitors[i], ANIMA_ENGINE_CONNECTED);
	}
...
const AnimaEngineWindowvidmode* _AnimaEngineWindowChooseVideoMode(_AnimaEngineWindowmonitor* monitor, const AnimaEngineWindowvidmode* desired)
{
	...
	for (i = 0; i < monitor->_modeCount; i++)
	{
		...
		if (desired->_refreshRate != ANIMA_ENGINE_DONT_CARE)
...
{
	_ANIMA_ENGINE_REQUIRE_INIT_OR_RETURN(NULL);
	_ANIMA_ENGINE_SWAP_POINTERS(AnimaEngine::_callbacks._monitor, cbfun, AnimaEngineWindowmonitorfun);
	return cbfun;
}
...
bool _AnimaEngineWindowRefreshContextAttribs(const _AnimaEngineWindowctxconfig* ctxconfig)
{
	...
	if (windowContext->_api == ANIMA_ENGINE_OPENGL_API)
	{
		...
		if (windowContext->_major > 3 || (windowContext->_major == 3 && windowContext->_minor >= 2))
		{
			...
			if (mask & GL_CONTEXT_COMPATIBILITY_PROFILE_BIT)
				windowContext->_profile = ANIMA_ENGINE_OPENGL_COMPAT_PROFILE;
			else if (mask & GL_CONTEXT_CORE_PROFILE_BIT)
				windowContext->_profile = ANIMA_ENGINE_OPENGL_CORE_PROFILE;
		...
		if (AnimaEngine::ExtensionSupported("GL_ARB_robustness"))
		{
			...
			if (strategy == GL_LOSE_CONTEXT_ON_RESET_ARB)
				windowContext->_robustness = ANIMA_ENGINE_LOSE_CONTEXT_ON_RESET;
			else if (strategy == GL_NO_RESET_NOTIFICATION_ARB)
				windowContext->_robustness = ANIMA_ENGINE_NO_RESET_NOTIFICATION;
		}
	}
	else
	{
	...
	if (AnimaEngine::ExtensionSupported("GL_KHR_context_flush_control"))
	{
		...
		if (behavior == GL_NONE)
			windowContext->_release = ANIMA_ENGINE_RELEASE_BEHAVIOR_NONE;
		else if (behavior == GL_CONTEXT_RELEASE_BEHAVIOR_FLUSH)
			windowContext->_release = ANIMA_ENGINE_RELEASE_BEHAVIOR_FLUSH;
	}
#endif
...
{
	if (ctxconfig->_api != ANIMA_ENGINE_OPENGL_API && ctxconfig->_api != ANIMA_ENGINE_OPENGL_ES_API)
	{
	...
	if (ctxconfig->_api == ANIMA_ENGINE_OPENGL_API)
	{
		...
		{
			if (ctxconfig->_profile != ANIMA_ENGINE_OPENGL_CORE_PROFILE && ctxconfig->_profile != ANIMA_ENGINE_OPENGL_COMPAT_PROFILE)
			{
	...
	else if (ctxconfig->_api == ANIMA_ENGINE_OPENGL_ES_API)
	{
		if ( ctxconfig->_major < 1 ||  ctxconfig->_minor < 0	||
	...
	{
		if (ctxconfig->_robustness != ANIMA_ENGINE_NO_RESET_NOTIFICATION && ctxconfig->_robustness != ANIMA_ENGINE_LOSE_CONTEXT_ON_RESET)
		{
	...
	{
		if (ctxconfig->_release != ANIMA_ENGINE_RELEASE_BEHAVIOR_NONE && ctxconfig->_release != ANIMA_ENGINE_RELEASE_BEHAVIOR_FLUSH)
		{
