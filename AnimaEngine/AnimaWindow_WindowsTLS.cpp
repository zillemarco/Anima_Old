...
{
	AnimaEngine::_GET_ANIMA_ENGINE_PLATFORM_LIBRARY_TLS_STATE->_context = TlsAlloc();
	if (AnimaEngine::_GET_ANIMA_ENGINE_PLATFORM_LIBRARY_TLS_STATE->_context == TLS_OUT_OF_INDEXES)
	{
		//_glfwInputError(GLFW_PLATFORM_ERROR, "Win32: Failed to allocate TLS index");
		return false;
	}

	AnimaEngine::_GET_ANIMA_ENGINE_PLATFORM_LIBRARY_TLS_STATE->_allocated = true;
	return true;
}

void _AnimaEngineWindowTerminateTLS(void)
{
	if (AnimaEngine::_GET_ANIMA_ENGINE_PLATFORM_LIBRARY_TLS_STATE->_allocated)
		TlsFree(AnimaEngine::_GET_ANIMA_ENGINE_PLATFORM_LIBRARY_TLS_STATE->_context);
}
