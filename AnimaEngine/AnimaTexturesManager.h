...
public:
	 ...
	 *	\param[out]	data			Parametro che verrà valorizzato con i dati del file. All'inizio il puntatore puntato da data deve essere NULL
	 								e viene allocatto all'interno della procedura. Il dato dovrà poi essere deallocato con la funzione free()
	 *	\param[out]	dataSize		Conterrà la lunghezza del buffer puntato da data
	 ...
	 *	\param[in]	filePath		Path del file da leggere
	 *	\param[out]	data			Parametro che verrà valorizzato con i dati del file. Il dato sarà un'array di dimensione imagesCount * mipMapsCount
									che conterrà i dati delle immagini e delle mip-map
	 *	\param[out]	dataSize		Conterrà un array di lunghezze dei buffer letti per ogni immagine e mip-map. Il dato punterà ad un'array di dimensione imagesCount * mipMapsCount.
	 *	\param[out]	imagesCount		Conterrà il numero di immagini lette (per una texture normale sarà 1, per una texture cube sarà 6 e così via)
	 *	\param[out]	width			Conterrà da larghezza dell'immagine
	 *	\param[out]	height			Conterrà l'altezza dell'immagine
	 *	\param[out]	depth			Conterrà la profondità dell'immagine
	 *	\param[out]	mipMapsCount	Conterrà il numero di mip-map memorizzate in ongi immagine letta
	 *	\param[out]	format			Conterrà il formato della texture, altrimenti 0
	 *	\param[out]	internalFormat	Conterrà il formato interno della texture, altrimenti 0
	 *	\param[out]	target			Conterrà il target della texture
	 *	\return		True nel caso la lettura abbia avuto successo, false altrimenti
	 *	\author		Zille Marco
